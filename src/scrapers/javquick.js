...
async function scrapeJavquickStreams(id) {
  try {
    ...
    if (body.startsWith("http://") || body.startsWith("https://")) {
      return [{ url: body, name: "Orange MP4", behaviorHints: { notWebReady: false, proxyHeaders: { request: { "User-Agent": UA, "Referer": "https://javquick.com/" } } } }];
    }
    ...
    if (decoded.includes("|")) {
      ...
      if (urls.length > 0) return urls.map(u => ({ url: u, name: "Orange MP4", behaviorHints: { notWebReady: false, proxyHeaders: { request: { "User-Agent": UA, "Referer": "https://javquick.com/" } } } }));
    }

    const pipeUrls = body.split("|").map(p => p.trim()).filter(p => p.startsWith("http://") || p.startsWith("https://"));
    if (pipeUrls.length > 0) return pipeUrls.map(u => ({ url: u, name: "Orange MP4", behaviorHints: { notWebReady: false, proxyHeaders: { request: { "User-Agent": UA, "Referer": "https://javquick.com/" } } } }));
